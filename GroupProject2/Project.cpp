...
void solvePath() {
	...
	do {
		...
		if ((int*)currNode->value < table[currNode->name].load().currLowestValue.load())
			table[currNode->name].load().currLowestValue.store((int*)currNode->value);
		if (!nodes[currNode->name].neighbors.empty()) {
			for each (GraphEdge x in nodes[currNode->name].neighbors) {
				queue.Enqueue(new Node(nodes[x.ID].value + x.weight, nodes[x.ID].name));
			}
		}
